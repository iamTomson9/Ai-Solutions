...
export default function SalesEventsScreen({ navigation }) {
  const [events, setEvents] = useState([]);
  useEffect(() => { const unsub = navigation.addListener('focus', load); return unsub; }, [navigation]);
  const load = async () => { setEvents(await DB.getEvents()); };

  const handleDelete = (id) => {
    Alert.alert('Delete Event', 'Are you sure you want to delete this event?', [
      { text: 'CANCEL', style: 'cancel' },
      { text: 'DELETE', style: 'destructive', onPress: async () => { await DB.deleteEvent(id); Alert.alert('Success', 'Event deleted successfully'); load(); } },
    ]);
  };
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.actions}>
        <TouchableOpacity style={styles.editBtn} onPress={() => navigation.navigate('CreateEvent', { event: item })}><Text style={styles.btnText}>Edit</Text></TouchableOpacity>
  ...
  return (
    <View style={styles.container}>
      <FlatList data={events} keyExtractor={item => item.id.toString()} renderItem={renderItem}
        contentContainerStyle={styles.list} ListEmptyComponent={<Text style={styles.empty}>No events created yet.</Text>} />
